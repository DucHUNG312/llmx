...
template <class Func, class Stride> struct CustomStride {
  ...
  make_layout(const Shape &shape, const CustomStride &stride) {
...
CUTLASS_HOST_DEVICE auto make_custom_stride_layout(Func &&func,
  ...
  return make_layout(repeat_like(shape, _1{}),
                     replace<I>(stride, CustomStride{static_cast<Func &&>(func),
                                                     get<I>(stride)}));
}

/// Helper function to optionally create a gather tensor
template <class Iterator, class Shape, class Stride, class Func>
CUTLASS_HOST_DEVICE auto make_gather_tensor(Iterator iter, const Shape &shape,
                                            const Stride &stride, Func &&func) {
...
template <int N, class OuterShape, class OuterStride, class Offset, class Shape,
          class Stride>
CUTE_HOST_DEVICE constexpr auto
upcast(ComposedLayout<Layout<OuterShape, OuterStride>, Offset,
...
template <class ShapeA, class StrideA, class OuterShapeB, class OuterStrideB,
          class OffsetB, class ShapeB, class StrideB>
CUTE_HOST_DEVICE constexpr auto
max_common_vector(Layout<ShapeA, StrideA> const &a,
                  ComposedLayout<Layout<OuterShapeB, OuterStrideB>, OffsetB,
