#pragma once
#include "llmx/cuda/common/math.h"
...
template <int ROWS_PER_THR, bool ALIBI, bool LOCAL> struct Mask {
  ...
  CUTE_HOST_DEVICE Mask(int q_len, int kv_len, const int group_size,
                        int sliding_window)
      : kv_len_(kv_len), group_size_(group_size),
        sliding_window_(sliding_window), diagonal_offset_(kv_len - q_len) {}

  // cS_mn: ((2, MMA_M), (2, MMA_N))
  template <typename IdentityS>
  CUTE_HOST_DEVICE void
  init_alibi(IdentityS &cS_mn, // ((2, MMA_M), (2, MMA_N)) => (M, N)
             int kv_head_idx, float sm_scale, const float *alibi_slops_ptr) {
  ...
  CUTE_HOST_DEVICE void
  apply(FragmentS &rS_mn, // ((2, MMA_M), (2, MMA_N))
