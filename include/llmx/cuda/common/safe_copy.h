...
template <class, class Enable = void> constexpr bool has_with_bool = false;

template <class Copy_Atom>
constexpr bool has_with_bool<
    Copy_Atom, cute::void_t<decltype(declval<typename Copy_Atom::Traits>().with(
                   declval<bool>()))>> = true;
...
select(const ComposedLayout<Swizzle<B, M, S>, Offset, LayoutB> &c) {
...
CUTE_HOST_DEVICE void zfill(const Copy_Atom &copy_atom, const TensorS &src,
                            TensorD &&dst) {
...
template <bool EVEN_MN, bool EVEN_K, bool ZFILL_MN, bool ZFILL_K,
          class CopyAtom, class TV, class Tiler, class TensorS, class TensorD,
          class TensorC, class Residue,
          __CUTE_REQUIRES(TensorS::rank == 3 && TensorD::rank == 3 &&
                          TensorC::rank == 3)>
CUTE_HOST_DEVICE void
safe_copy(const TiledCopy<CopyAtom, TV, Tiler> &tiled_copy,
...
safe_copy(const TiledCopy<CopyAtom, TV, Tiler> &tiled_copy,
  ...
  safe_copy<EVEN_MN, EVEN_K, ZFILL_MN, ZFILL_K>(tiled_copy, src, dst, identity,
                                                residue);
}
...
safe_copy(const TiledCopy<CopyAtom, TV, Tiler> &tiled_copy,
  ...
  for (int k = 0; k < size<3>(src); ++k) {
    safe_copy<EVEN_MN, EVEN_K, ZFILL_MN, ZFILL_K>(
        tiled_copy, src(_, _, _, k), dst(_, _, _, k), identity(_, _, _, k),
        residue);
  }
}

template <bool EVEN_K, bool ZFILL_K, class CopyAtom, class TV, class Tiler,
          class TensorS, class TensorD, class TensorC, class Residue,
