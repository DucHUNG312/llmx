...
template <int N, typename T> CUTE_DEVICE T group_reduce_sum(T val) {
...
template <int N, typename T> CUTE_DEVICE T group_reduce_max(T val) {
  static_assert((N & (N - 1)) == 0, "N must be power of 2 ");
  CUTE_UNROLL
  for (int mask = N / 2; mask > 0; mask >>= 1) {
...
template <int ROWS_PER_THR> struct OnlineSoftmax {
  ...
  template <typename EngineS, typename LayoutS, typename EngineO,
            typename LayoutO, typename ReduceFunc>
